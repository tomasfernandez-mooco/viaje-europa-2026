  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
      <div className="animate-fade-in flex items-end justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-stone-600 glass-card rounded-xl hover:bg-white/60 transition-all"
            title="Cambiar orden"
          >
      ...
      <div className="glass-card rounded-2xl p-4 animate-fade-in flex flex-wrap gap-3 items-center" style={{ animationDelay: "0.02s" }}>
        <span className="text-xs font-medium text-stone-400 uppercase tracking-wider">Filtrar:</span>
        <select
          value={filterCiudad}
        ...
        {hasFilters && (
          <button
            ...
            className="text-xs text-stone-500 hover:text-accent transition-colors px-2 py-1 rounded-lg hover:bg-white/40"
          >
            Limpiar filtros ✕
          </button>
        )}
          ...
          <div className="flex glass-card rounded-xl p-1 gap-1">
            {(["categoria", "destino"] as const).map((tab) => (
              <button
                ...
                className={`px-4 py-1.5 text-xs font-medium rounded-lg transition-all ${chartToggle === tab ? "bg-accent text-white shadow-glass-sm" : "text-stone-500 hover:text-stone-700"}`}
              >
      ...
      <div className="glass-card rounded-2xl overflow-hidden animate-fade-in" style={{ animationDelay: "0.2s" }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/20">
          <h2 className="text-sm font-semibold text-stone-700">Detalle por categoría</h2>
          <span className="text-xs text-stone-400">{filtered.length} reservas</span>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/20">
              <th className="text-left px-5 py-3 text-[11px] font-semibold text-stone-400 uppercase tracking-wider">Categoria</th>
              <th className="text-right px-5 py-3 text-[11px] font-semibold text-stone-400 uppercase tracking-wider cursor-pointer hover:text-accent" onClick={() => setSortDir(d => d === "desc" ? "asc" : "desc")}>
                Total USD {sortDir === "desc" ? "↓" : "↑"}
              </th>
              <th className="text-right px-5 py-3 text-[11px] font-semibold text-stone-400 uppercase tracking-wider">Por persona</th>
              <th className="text-right px-5 py-3 text-[11px] font-semibold text-stone-400 uppercase tracking-wider">% del total</th>
            </tr>
          </thead>
